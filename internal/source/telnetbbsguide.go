...
func (a *TelnetBBSGuide) Fetch(ctx context.Context) ([]Entry, error) {
	...
	for _, letter := range "abcdefghijklmnopqrstuvwxyz" {
		...
		if err != nil { return nil, fmt.Errorf("fetch %s: %w", u, err) }
		if resp.StatusCode != http.StatusOK { resp.Body.Close(); return nil, fmt.Errorf("fetch %s: HTTP %s", u, resp.Status) }
		entries, err := parseTelnetBBSGuide(resp.Body, u)
		resp.Body.Close()
		if err != nil { return nil, fmt.Errorf("parse %s: %w", u, err) }
...
func parseTelnetBBSGuide(r interface{ Read([]byte) (int, error) }, pageURL string) ([]Entry, error) {
	...
	for i := 0; i < len(lines); i++ {
		if !strings.HasPrefix(lines[i], "Telnet:") { continue }
		endpoint := strings.TrimSpace(strings.TrimPrefix(lines[i], "Telnet:"))
		if endpoint == "" && i+1 < len(lines) { i++; endpoint = strings.TrimSpace(lines[i]) }
		if endpoint == "" { continue }
		name := previousName(lines, i)
		software := ""
		for j := i+1; j < len(lines) && j < i+8; j++ {
			if strings.HasPrefix(lines[j], "Software:") { software = strings.TrimSpace(strings.TrimPrefix(lines[j], "Software:")); break }
		}
		host, port := splitEndpoint(endpoint, 23)
		if host == "" { continue }
		key := strings.ToLower(host) + ":" + strconv.Itoa(port)
		out = append(out, Entry{Source:"telnetbbsguide", SourceKey:key, SourceURL:pageURL, Name:name, Software:software, Protocol:"telnet", Hostname:host, Port:port})
	}
	return out, nil
}
...
func textLines(n *html.Node) []string {
	...
	walk = func(x *html.Node) {
		if x.Type == html.TextNode {
			for _, s := range strings.Split(x.Data, "\n") { if s = strings.TrimSpace(s); s != "" { raw = append(raw, s) } }
		}
...
func previousName(lines []string, idx int) string {
	for i := idx-1; i >= 0 && i >= idx-8; i-- {
		...
		if s == "Image" || strings.Contains(s, ":") || s == "MORE..." { continue }
		if len(s) > 1 { return s }
	}
	return "Unknown BBS"
}
...
func splitEndpoint(s string, defaultPort int) (string, int) {
	...
	if u, err := url.Parse("//" + s); err == nil {
		h := u.Hostname(); p := defaultPort
		if ps := u.Port(); ps != "" { if n, err := strconv.Atoi(ps); err == nil { p = n } }
		return h, p
	}
